...
(function (root, factory) {
  ...
  function applyThermoFill(gauge, pct, color, { animate = true, fromZero = false } = {}) {
    ...
    const bulbFill = gauge.querySelector(".vote-thermo-gauge__bulb-fill");
    ...
    const applyTarget = () => {
      ...
      if (bulbFill) {
        bulbFill.style.backgroundColor = color;
      }
    ...
    return `
      ...
      >
        ...
        <div class="vote-thermo-gauge__bulb" aria-hidden="true">
          <span class="vote-thermo-gauge__bulb-fill" style="background-color:${color};"></span>
        </div>
      </div>
    `;
  }
  ...
  function mountOrUpdateThermoGauge(cardOrEl, passPct, { animate = true } = {}) {
    ...
    let isNew = !gauge;
    // Upgrade legacy slim gauges (no glass tube / bulb) in place.
    if (gauge && !gauge.querySelector(".vote-thermo-gauge__tube")) {
      const ready = gauge.classList.contains("is-ready");
      gauge.outerHTML = buildThermoGaugeHtml(ready || !animate ? pct : 0);
      gauge = host.querySelector(".vote-thermo-gauge");
      isNew = true;
    }
    if (!gauge) {
