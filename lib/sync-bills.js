...
function mapBillToProcessedRow(bill, card = null, extras = {}) {
  ...
  if (card) {
    ...
    if (card.short_title) row.short_title = card.short_title;
    if (card.plain_summary || card.card_summary || card.summary) {
      row.plain_summary = card.plain_summary || card.card_summary || card.summary;
      row.card_summary = card.card_summary || card.plain_summary || card.summary;
      row.what_it_does = card.what_it_does || row.plain_summary;
    }
    if (card.yea_impact) row.yea_impact = card.yea_impact;
    if (card.nay_impact) row.nay_impact = card.nay_impact;
    if (card.takeaway) row.takeaway = card.takeaway;
    if (card.onboarding_question) {
      row.onboarding_question = card.onboarding_question;
    }
    if (card.pro_argument) row.pro_argument = card.pro_argument;
    if (card.con_argument) row.con_argument = card.con_argument;
    if (card.key_points) row.key_points = card.key_points;
    if (card.primary_category) row.primary_category = card.primary_category;
    if (typeof card.is_key_vote === "boolean") row.is_key_vote = card.is_key_vote;
...
async function processOneBill(bill, supabase, congressApiKey, options = {}) {
  ...
  // Prefer Claude Action Match cards whenever possible. CRS text is useful
  // source material, but raw CRS dumps make poor scorecard summaries.
  try {
    ...
    if (!force && !isGenericOfficialSummary(officialSummary, title)) {
      ...
      return {
        status: "crs",
        billId,
        message: claudeError.message || "Claude failed; stored CRS",
      };
    }
    throw claudeError;
  }
...
async function syncAllBills(options = {}) {
  ...
  while (results.fetched < max) {
    ...
    const url = `${CONGRESS_API}/bill/${congress}?limit=${pageSize}&offset=${offset}&sort=updateDate+desc&format=json&api_key=${encodeURIComponent(
      congressApiKey
    )}`;
