...
const SYSTEM_PROMPT = `You write plain-English vote cards for a civic app. Write at an 8th-grade reading level: short words, clear sentences, no jargon.
...
1. Analyze ONLY the provided bill title + congressional/CRS text. Do not invent programs, repeals, bans, dollar amounts, or funding cuts that are not clearly in the text.
2. Be CONCRETE, not vague. Prefer hard facts from the text: dollar amounts, dates, agency names, who must do what, exact rules, caps, eligibility cuts, and penalties/fines. Name the actual programs, offices, or people affected when the text says so.
3. Ban empty buzzwords and filler. Do NOT write vague phrases like "provides funding for", "addresses", "supports", "improves", "strengthens", "takes steps to", "aims to", or "makes changes related to" unless you immediately follow with the specific dollar amount, rule, agency directive, or penalty from the text.
4. summary must be 2–3 COMPLETE sentences in plain English (no legalese).
   - Cover the exact legal or numeric mechanism (what is authorized, required, banned, funded, cut, or redirected — with numbers when present).
   - Cover the direct real-world impact on citizens, taxpayers, workers, businesses, or communities if this vote prevails.
   - Do NOT restate or paraphrase the bill title.
5. yea_means / nay_means must be exactly 1 sentence each describing the concrete real-world outcome of Yea vs Nay (include numbers/rules when available).
6. yea_label / nay_label must be 2–3 words, parallel, concrete (e.g. "End Rebates" / "Keep Rebates"). If unsure, use "Support Measure" / "Oppose Measure".
7. No slogans and no fear-mongering.`;

const VOTE_CARD_TOOL = {
  name: "submit_vote_card",
  description:
    "Submit the plain-English vote card with concrete details (dollars, agencies, rules, penalties). Always call this tool with valid fields; never reply with free-form text.",
  input_schema: {
    type: "object",
    ...
    properties: {
      summary: {
        type: "string",
        description:
          "2–3 plain-English sentences (8th-grade level). Must cover (1) the exact legal/numeric mechanism from the text — dollar amounts, agency directives, rules, caps, or penalties when present — and (2) the direct impact on citizens. Ban vague buzzwords like 'provides funding for' or 'addresses'. Do not repeat the bill title.",
      },
      yea_means: {
        type: "string",
        description:
          "One sentence: concrete real-world outcome of a Yea vote (include numbers/rules when in the text).",
      },
      nay_means: {
        type: "string",
        description:
          "One sentence: concrete real-world outcome of a Nay vote (include numbers/rules when in the text).",
      },
      yea_label: {
        type: "string",
...
- summary: 2–3 sentences covering the exact legal/numeric mechanism (dollars, agencies, rules, penalties) and direct citizen impact — no vague buzzwords; do not copy the title
- yea_means / nay_means: 1 concrete sentence each
