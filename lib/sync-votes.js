...
{"summary":"string","yea_means":"string","nay_means":"string","yea_label":"string","nay_label":"string","short_title":"string","what_it_does":"string","yea_impact":"string","nay_impact":"string"}
...
6. short_title must be a short plain-English topic (3–7 words), Title Case, with NO bill numbers and NO "Amdt. No." codes.
   Good: "Local Police Hiring Grants". Bad: "Cortez Masto Amdt. No. 5463".
7. what_it_does must be exactly 1 sentence on real-world impact (can overlap summary sentence 2).
8. yea_impact / nay_impact must be exactly 1 sentence each on what Support/Yea vs Oppose/Nay advocates for.
  ...
  input_schema: {
    ...
    properties: {
      ...
      short_title: {
        type: "string",
        description:
          "3–7 word plain-English topic title. No bill numbers or amendment codes.",
      },
      what_it_does: {
        type: "string",
        description: "One sentence: real-world impact of the measure.",
      },
      yea_impact: {
        type: "string",
        description: "One sentence: what a Support / Yea vote advocates for.",
      },
      nay_impact: {
        type: "string",
        description: "One sentence: what an Oppose / Nay vote advocates for.",
      },
    },
    required: [
      "summary",
...
function normalizeCard(parsed, fallbackTitle) {
  ...
  let nay_label = String(parsed?.nay_label || "")
    ...
    .slice(0, 3)
    .join(" ");
  const what_it_does = String(parsed?.what_it_does || "").trim();
...
function buildUserPrompt(title, rawText) {
  return `Bill title (for context only — do not copy it into short_title or summary): ${
...
- short_title: 3–7 word plain-English topic (no Amdt./bill codes)
- what_it_does: 1 sentence real-world impact
- summary: exactly 2 sentences — what changes + practical impact on people (not the title)
- yea_means / nay_means: 1 sentence each
- yea_impact / nay_impact: 1 sentence each on what Support vs Oppose advocates for
...
async function upsertProcessedVote(supabase, row) {
  ...
  if (
    ...
    // Migration not applied yet — persist legacy fields so sync still succeeds.
    const legacy = { ...row };
