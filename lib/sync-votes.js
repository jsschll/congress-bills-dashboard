...
const SYSTEM_PROMPT = `You are a plain-language civic journalist explaining Senate votes to an everyday voter. Your goal is to translate procedural amendments into 1-2 sentences explaining what policy issue is being voted on in plain English.
...
- DEFINE THE ACTION: State clearly what a YEA vote is trying to fund, change, or protect in real-life terms.
- EXPLAIN THE OPPOSITION: State briefly why someone would vote NAY (e.g., opposing new federal spending, procedural objections, or party line votes).
...
EXPECTED OUTPUT JSON STRUCTURE (JSON only — no markdown, no code fences, no prose before or after):
{
  ...
  "plain_summary": "1-2 sentence breakdown of what this amendment actually changes or funds.",
  "yea_impact": "Votes to allocate federal funding for local departments to hire officers.",
  "nay_impact": "Votes against adding this extra spending program to the federal bill."
}`;
  ...
  input_schema: {
    ...
    properties: {
      ...
      plain_summary: {
        type: "string",
        description:
          "1–2 sentence breakdown of what this amendment actually changes or funds in real life.",
      },
      yea_impact: {
        type: "string",
        description:
          'What a YEA vote tries to fund, change, or protect. Example: "Votes to allocate federal funding for local departments to hire officers."',
      },
      nay_impact: {
        type: "string",
        description:
          'Why someone would vote NAY. Example: "Votes against adding this extra spending program to the federal bill."',
      },
    },
...
- plain_summary: 1–2 sentences on what this amendment actually changes or funds
- yea_impact: 1 sentence on what a YEA vote tries to fund/change/protect
- nay_impact: 1 sentence on why someone would vote NAY

Ban jargon. Explain the policy issue for an everyday voter. Return structured tool input only.`;
}
...
async function formatVoteWithAnthropic(context = {}) {
  ...
  return normalizeCard(parsed, fallbackTitle);
}
