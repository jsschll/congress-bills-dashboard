...
Format your response in plain, direct English. Use Parent Bill + Amendment context, but never paste amendment numbers or process jargon. Do not invent programs, funding amounts, repeals, or bans that are not clearly in the source text. No slogans and no fear-mongering.
...
EXPECTED OUTPUT JSON STRUCTURE (JSON only — no markdown, no code fences, no prose before or after):
{
  ...
  "key_points": [
    "Adds major border security funding for agents and barriers",
    "Affects communities and workers near the southern border",
    "Raises federal spending on enforcement programs"
  ],
  "pro_argument": "Supporters say the money is needed to secure the border and reduce illegal crossings.",
  "con_argument": "Opponents say the package spends too much and skips broader immigration reforms.",
...
const VOTE_CARD_TOOL = {
  ...
  description:
    "Submit the structured plain-language vote breakdown. Always call this tool with valid fields; never reply with free-form text.",
  input_schema: {
    type: "object",
    ...
    properties: {
      ...
      card_summary: {
        type: "string",
        description:
          "Strict 2-sentence summary max (~35 words) for the scorecard accordion. Real-world impact only — no jargon.",
      },
      takeaway: {
        type: "string",
        description:
          "1-line crisp takeaway headline for the deep-dive Bill Breakdown view.",
      },
      key_points: {
        type: "array",
        minItems: 3,
        ...
        items: { type: "string" },
        description:
          "Exactly 3 short bullets: real-world impact, who is affected, and fiscal/legal change.",
      },
      pro_argument: {
        type: "string",
        description:
          "1 sentence on why supporters voted Yea / back this measure.",
      },
      con_argument: {
        type: "string",
        description:
          "1 sentence on why opponents voted Nay / oppose this measure.",
      },
      primary_category: {
        type: "string",
...
function normalizePrimaryCategory(value, context = {}) {
  ...
  const aliasMap = [
    ...
    [/hous|rent|mortgage|homeless|infra|transit|highway|bridge/, "Housing & Infrastructure"],
    [/foreign|defense|military|war|nato|troop|sanction/, "Foreign Policy & Defense"],
    [/civil|justice|voting|police|prison|gun|court|rights/, "Civil Rights & Justice"],
    [/energy|environ|climate|epa|emission|oil|gas|renewable/, "Energy & Environment"],
    [/educat|school|student|labor|union|wage|worker|osha/, "Education & Labor"],
    [/tax|budget|economy|spend|deficit|debt|tariff|irs|fee|payroll/, "Economy & Taxes"],
  ];
...
function normalizeCard(parsed, fallbackTitle, context = {}) {
  ...
  const takeawayFallback =
    takeaway || short_title || "Congressional roll-call vote";
...
- card_summary: ≤2 sentences AND ≤35 words for the scorecard accordion
- takeaway: 1-line crisp headline for the deep-dive breakdown
- key_points: exactly 3 short bullets (impact, who is affected, fiscal/legal change)
- pro_argument: 1 sentence on why supporters voted Yea
- con_argument: 1 sentence on why opponents voted Nay
...
Ban jargon. Keep every field punchy and scannable. Return structured tool input only.`;
}
