...
const { classifyVoteKind } = require("./format-bill-summary");
const {
...
- BAN EMPTY BUZZWORDS: Do NOT write filler like "provides funding for", "addresses", "aims to", or "makes changes related to" unless you immediately follow with the specific dollar amount, rule, agency directive, or penalty from the text.
- FOCUS ON REAL-WORLD IMPACT: Explain what changes for an ordinary citizen's wallet, rights, or community.
- MAXIMUM LENGTHS: card_summary ≤2 short complete sentences (~55 words). takeaway = 1 crisp headline line. key_points = exactly 2 KEY IMPACT bullets (each under 10 words, each starting with an active verb). pro_argument / con_argument = 1 complete sentence each. onboarding_question ≤20 words, one complete question.
- KEY IMPACTS (key_points): Follow these critical rules exactly:
  1. START WITH VERBS: Every bullet MUST begin with a strong active verb (Boosts, Establishes, Cuts, Protects, Updates, Increases, Requires, Limits, etc.).
  2. NO LEGALESE: Plain English real-world outcomes in under 10 words per bullet.
  3. NO REPETITION: Do NOT repeat the bill title or official name inside the bullets.
  4. ACCURACY: Base bullets strictly on the provided bill text. Do NOT invent details or use generic placeholders.
- NEVER TRUNCATE: Finish every sentence and noun phrase. Do not cut mid-word or mid-phrase (bad: "convicted of sexual"; good: "convicted of sexual assault").
- ONBOARDING QUESTION: Write a complete, standalone, natural English question a voter can answer Support/Oppose/Skip. Max 20 words. Must end with "?". Do NOT include bill numbers (no "HR 134"), "which …" relative clauses, or truncated stubs. Good examples: "Do you support requiring mandatory detention and deportation for non-citizens convicted of violent crimes?" / "Should Congress block proposed EPA restrictions on coal-fired power plants?" / "Do you support overriding local D.C. criminal justice tax code revisions?"
...
EXPECTED OUTPUT JSON STRUCTURE (JSON only — no markdown, no code fences, no prose before or after):
{
  ...
  "onboarding_question": "Do you support adding $70 billion for border security agents and barriers?",
  "key_points": [
    "Boosts border agent hiring with new funds",
...
KEY IMPACTS reference (also acceptable as key_impacts):
${KEY_IMPACTS_SYSTEM_PROMPT}`;
  ...
  input_schema: {
    ...
    properties: {
      ...
      onboarding_question: {
        type: "string",
        description:
          'Complete standalone natural-English Voter Pulse quiz question, max 20 words, ending with "?". Examples: "Do you support requiring mandatory detention and deportation for non-citizens convicted of violent crimes?" / "Should Congress block proposed EPA restrictions on coal-fired power plants?". No bill numbers, no "which …" stitching, never truncate mid-word.',
      },
      key_points: {
        type: "array",
        minItems: 2,
        ...
        items: { type: "string" },
        description:
          "Exactly 2 KEY IMPACT bullets. Each must start with an active verb (Boosts/Establishes/Cuts/Protects/Updates/...), stay under 10 words, describe a real-world outcome from the bill text, and never repeat the bill title or use placeholders.",
      },
      pro_argument: {
        type: "string",
        description:
...
async function fetchLatestHouseVotes(apiKey, { congress, limit }) {
  ...
  // List responses omit voteQuestion; hydrate from detail endpoints.
  const hydrated = [];
  const chunkSize = 6;
  for (let i = 0; i < selected.length; i += chunkSize) {
    ...
    const rows = await Promise.all(
      chunk.map(async (raw) => {
        const base = mapHouseVote(raw);
        if (base.vote_question) return base;
        try {
          const detailUrl = `${CONGRESS_API}/house-vote/${base.congress}/${base.session_number}/${base.roll_call_number}?format=json&api_key=${encodeURIComponent(
            apiKey
          )}`;
          const detail = await fetchJson(detailUrl);
          const vote = detail.houseRollCallVote || {};
          const voteQuestion = String(
            vote.voteQuestion || vote.question || ""
          ).trim();
          if (voteQuestion) {
            base.vote_question = voteQuestion;
            if (!raw.legislationTitle && voteQuestion) {
              base.title = raw.legislationTitle || voteQuestion || base.title;
            }
            const voteKind = classifyVoteKind(voteQuestion, base.result, {
              legislationType: base.bill_type,
              billNumber: base.bill_number,
              title: base.title,
            });
            base.vote_kind = voteKind;
            base.raw_payload = {
          ...
          console.warn(
            "house vote detail hydrate failed:",
            error.message || error
          );
        }
        return base;
...
/** Cap plain_summary at 2 complete sentences for scannable cards. */
function clampPlainSummary(text = "", { maxSentences = 2, maxWords = 55 } = {}) {
  ...
  if (words.length > maxWords) {
    // Prefer one full sentence over chopping mid-phrase.
...
/**
 * Normalize Claude's Voter Pulse quiz question: ≤20 words, ends with "?",
 * never mid-phrase truncated.
 */
function clampOnboardingQuestion(text = "", { maxWords = 20 } = {}) {
  ...
  out = out
    ...
    .replace(/\s+/g, " ")
    .trim();
  // Salvage old "Do you support HR 134, which requires …?" templates.
  const whichSalvage = out.match(
    /^(?:do you support|should congress)\s+(?:h\.?\s*r\.?|s\.?|h\.?\s*j\.?\s*res\.?|s\.?\s*j\.?\s*res\.?|h\.?\s*con\.?\s*res\.?|s\.?\s*con\.?\s*res\.?)\s*\d+\s*,?\s*which\s+(.+)$/i
  );
  if (whichSalvage) {
    const clause = collapseWs(whichSalvage[1]).replace(/[.!?]+$/, "");
    out = clause ? `Do you support a measure that ${clause}?` : "";
  }
  out = collapseWs(out);
  if (!out) return "";

  // Keep a single question / sentence.
  const qMatch = out.match(/^[^?]+\?/);
  if (qMatch) out = collapseWs(qMatch[0]);
  else {
    const first = splitPlainSentences(out)[0] || out;
    out = collapseWs(first.replace(/[.!]+$/, ""));
  }
  ...
  if (words.length > maxWords) {
    const weak =
      /^(of|the|a|an|and|or|for|to|with|by|in|on|at|from|into|that|which|who|their|its)$/i;
    let end = maxWords;
    while (end > Math.floor(maxWords * 0.7) && weak.test(words[end - 1] || "")) {
      end -= 1;
    }
  ...
  if (/\b[A-Z]\?$/.test(out)) return "";
  if (/\b(of|the|a|an|and|or|for|to|with|by|in|on|which|that)\?$/i.test(out)) {
    return "";
  }
...
/** Cap impact fields to one complete clause (~28 words). */
function clampImpactLine(text = "", { maxWords = 28 } = {}) {
  let out = collapseWs(text);
  if (!out || isUnknownClaudeValue(out)) return "";
  out = out
...
function normalizeKeyPoints(value, fallbacks = [], context = {}) {
  return normalizeKeyImpacts(value, {
    title: context.title || context.fallbackTitle || "",
    category: context.category || context.primary_category || "",
    sourceText: context.sourceText || context.rawText || context.cardSummary || "",
    fallbacks,
  });
}
...
function normalizeCard(parsed, fallbackTitle, context = {}) {
  ...
  const key_points = normalizeKeyPoints(
    parsed?.key_impacts || parsed?.key_points,
    [cardSummaryFallback, yeaFallback, nayFallback],
    {
      title: fallbackTitle || short_title || "",
      category: primary_category,
      sourceText: `${context.rawText || ""} ${cardSummaryFallback}`.trim(),
      cardSummary: cardSummaryFallback,
      fallbackTitle,
  ...
  const onboarding_question =
    clampOnboardingQuestion(parsed?.onboarding_question || "") ||
    clampOnboardingQuestion(
      takeawayFallback
        ? `Do you support ${takeawayFallback.charAt(0).toLowerCase()}${takeawayFallback
            .slice(1)
            .replace(/[.!?]+$/, "")}?`
        : ""
    ) ||
    clampOnboardingQuestion(
      short_title ? `Do you support the measure on ${short_title}?` : ""
    ) ||
    "Do you support this congressional measure?";
...
- card_summary: ≤2 complete sentences AND ~55 words; cover the exact legal/numeric mechanism (dollars, agencies, rules, penalties) and direct citizen impact — no vague buzzwords; do not copy the title; never truncate mid-phrase
- takeaway: 1-line crisp concrete headline for the deep-dive breakdown
- onboarding_question: complete standalone voter quiz question, max 20 words, ending with "?"; examples: "Do you support requiring mandatory detention and deportation for non-citizens convicted of violent crimes?" / "Should Congress block proposed EPA restrictions on coal-fired power plants?" — no bill numbers, no "which …" clauses, never truncate mid-word
- key_points: exactly 2 KEY IMPACT bullets; each starts with an active verb (Boosts/Establishes/Cuts/Protects/Updates/...), under 10 words, grounded in the source text, never repeats the bill title, never uses placeholders
- pro_argument: 1 complete concrete sentence on why supporters voted Yea
- con_argument: 1 complete concrete sentence on why opponents voted Nay
...
Write at an 8th-grade reading level. Ban jargon and empty filler ("provides funding for", "aims to") unless followed by a specific fact from the text. Always finish every sentence and noun phrase. Keep every field punchy and scannable. Return structured tool input only.`;
}
...
const BREAKDOWN_FIELD_ERROR_RE =
  /card_summary|takeaway|onboarding_question|key_points|pro_argument|con_argument|primary_category/i;

const IMPACT_FIELD_ERROR_RE =
  /short_title|plain_summary|what_it_does|yea_impact|nay_impact|is_key_vote|card_summary|takeaway|onboarding_question|key_points|pro_argument|con_argument|primary_category/i;

function withoutBreakdownFields(row) {
  const legacy = { ...row };
  delete legacy.card_summary;
  delete legacy.takeaway;
  delete legacy.onboarding_question;
...
async function upsertProcessedVote(supabase, row) {
  ...
  if (error && IMPACT_FIELD_ERROR_RE.test(error.message || "")) {
    ...
    let legacy = { ...row };
    if (/onboarding_question/i.test(error.message || "")) {
      delete legacy.onboarding_question;
    } else if (/is_key_vote/i.test(error.message || "")) {
      delete legacy.is_key_vote;
  ...
  for (const vote of mapped) {
    try {
      ...
      const row = {
        ...
        onboarding_question: card.onboarding_question || null,
