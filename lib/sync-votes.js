...
const SYSTEM_PROMPT = `You are a plain-language civic journalist. Your job is to translate complex congressional roll-call votes into simple, high-impact summaries that any voter can understand in 5 seconds.

RULES FOR SUMMARIZATION:
1. BAN ALL LEGALESE: Never use words like 'appropriations', 'reconciliation', 'pursuant', 'rescissions', 'heretofore', 'amendment no.', or 'procedural motion'.
2. LEAD WITH ACTION: Start with what the bill or vote ACTUALLY DOES in the real world (e.g., 'Provides $50M for local police officer hiring', NOT 'An amendment relating to law enforcement funding').
3. EXPLAIN THE VOTE STANCES:
   - \`yea_impact\`: What a YES vote means in plain English (e.g., 'Votes to give local departments money to hire more officers.').
   - \`nay_impact\`: What a NO vote means in plain English (e.g., 'Votes against adding this spending to the budget package.').
4. Keep \`short_title\` under 6 words (e.g., 'Local Police Hiring Grants').

Do not invent programs, funding amounts, repeals, or bans that are not clearly in the source text. Prefer concrete people, places, and money over process jargon. No slogans and no fear-mongering.

OUTPUT FORMAT (JSON only — no markdown, no code fences, no prose before or after):
{
  "short_title": "Short title",
  "plain_summary": "1-2 sentence real-world summary",
  "yea_impact": "1 sentence on Yea vote",
  "nay_impact": "1 sentence on Nay vote"
}`;
  ...
  input_schema: {
    ...
    properties: {
      short_title: {
        type: "string",
        description:
          'Under 6 words, Title Case, plain-English topic. Example: "Local Police Hiring Grants". No bill numbers or amendment codes.',
      },
      plain_summary: {
        type: "string",
        description:
          "1–2 sentence real-world summary that leads with action. Example: \"Provides $50M for local police officer hiring.\"",
      },
      yea_impact: {
        type: "string",
        description:
          "1 sentence on what a YES vote means. Example: \"Votes to give local departments money to hire more officers.\"",
      },
      nay_impact: {
        type: "string",
        description:
          "1 sentence on what a NO vote means. Example: \"Votes against adding this spending to the budget package.\"",
      },
    },
    required: ["short_title", "plain_summary", "yea_impact", "nay_impact"],
  },
};
...
function normalizeCard(parsed, fallbackTitle) {
  ...
  const yea_impact = String(
    parsed?.yea_impact || parsed?.yea_means || ""
  ).trim();
  const nay_impact = String(
  ...
  const yeaFallback =
    yea_impact ||
    "Votes yes to advance this measure as written.";
  const nayFallback =
    nay_impact || "Votes no to reject this measure.";
...
- short_title: under 6 words (e.g. "Local Police Hiring Grants")
- plain_summary: 1–2 sentence real-world summary that leads with action
- yea_impact: 1 sentence on what a YES vote means
- nay_impact: 1 sentence on what a NO vote means

Ban legalese. Lead with what it actually does for voters. Return structured tool input only.`;
}
