...
{"short_title":"string","plain_summary":"string","yea_impact":"string","nay_impact":"string","summary":"string","yea_means":"string","nay_means":"string","yea_label":"string","nay_label":"string"}

Content rules (strict):
1. Analyze ONLY the provided bill title + congressional/CRS / statement-of-purpose text. Do not invent programs, repeals, bans, or funding cuts that are not clearly in the text.
2. Be SPECIFIC. Name the actual programs, funding, rules, agencies, or people affected when the text says so.
3. short_title must be a short plain-English topic (3–7 words), Title Case, with NO bill numbers and NO "Amdt. No." codes.
   Good: "Local Police Hiring Grants". Bad: "Cortez Masto Amdt. No. 5463".
4. plain_summary must be exactly 1 COMPLETE sentence in plain English describing what the measure does.
   Good: "Allocates federal funding to help local departments hire law enforcement officers."
5. yea_impact / nay_impact must be exactly 1 sentence each on what Support/Yea vs Oppose/Nay advocates for.
   Good yea_impact: "Supports expanding federal funding for local police hiring."
   Good nay_impact: "Opposes adding new spending to the current budget bill."
6. summary must be exactly 2 COMPLETE sentences in plain English (no legalese).
   - Sentence 1: what the bill/measure actually changes — do NOT restate or paraphrase the bill title.
   - Sentence 2: the practical impact on citizens, taxpayers, workers, businesses, or communities.
7. yea_means / nay_means must be exactly 1 sentence each describing the real-world outcome of Yea vs Nay.
8. yea_label / nay_label must be 2–3 words, parallel, concrete (e.g. "End Rebates" / "Keep Rebates"). If unsure, use "Support Measure" / "Oppose Measure".
9. No slogans and no fear-mongering.`;
  ...
  input_schema: {
    ...
    properties: {
      short_title: {
        type: "string",
        description:
          '3–7 word plain-English topic title. Example: "Local Police Hiring Grants". No bill numbers or amendment codes.',
      },
      plain_summary: {
        type: "string",
        description:
          'Exactly 1 plain-English sentence on what the measure does. Example: "Allocates federal funding to help local departments hire law enforcement officers."',
      },
      yea_impact: {
        type: "string",
        description:
          'One sentence: what a Support / Yea vote advocates for. Example: "Supports expanding federal funding for local police hiring."',
      },
      nay_impact: {
        type: "string",
        description:
          'One sentence: what an Oppose / Nay vote advocates for. Example: "Opposes adding new spending to the current budget bill."',
      },
      summary: {
        type: "string",
        description:
...
function normalizeCard(parsed, fallbackTitle) {
  ...
  let nay_label = String(parsed?.nay_label || "")
    ...
    .slice(0, 3)
    .join(" ");
  // Accept plain_summary (canonical) or legacy what_it_does from older prompts.
  const plain_summary = String(
    parsed?.plain_summary || parsed?.what_it_does || ""
  ).trim();
  const yea_impact = String(parsed?.yea_impact || "").trim();
  const nay_impact = String(parsed?.nay_impact || "").trim();
  ...
  const plainSummaryFallback =
    plain_summary || firstSentence(summaryFallback) || summaryFallback;
  ...
  return {
    ...
    yea_impact: yea_impact || yeaFallback,
    nay_impact: nay_impact || nayFallback,
  };
}

function buildUserPrompt(title, rawText) {
  return `Bill title (for context only — do not copy it into short_title or plain_summary): ${
    title || "Untitled measure"
  }

Raw congressional / CRS / statement-of-purpose text:
"""
${String(rawText || "").slice(0, 6000) || "(No CRS summary available.)"}
"""

Call submit_vote_card with these required structured fields:
- short_title: 3–7 word plain-English topic (no Amdt./bill codes). Example: "Local Police Hiring Grants"
- plain_summary: 1 sentence on what it does. Example: "Allocates federal funding to help local departments hire law enforcement officers."
- yea_impact: 1 sentence. Example: "Supports expanding federal funding for local police hiring."
- nay_impact: 1 sentence. Example: "Opposes adding new spending to the current budget bill."
...
const IMPACT_FIELD_ERROR_RE =
  /short_title|plain_summary|what_it_does|yea_impact|nay_impact/i;
...
async function upsertProcessedVote(supabase, row) {
  ...
  if (error && IMPACT_FIELD_ERROR_RE.test(error.message || "")) {
    // Prefer dropping only plain_summary when that column is missing but
    // what_it_does / other impact columns already exist.
    let legacy = { ...row };
    if (/plain_summary/i.test(error.message || "")) {
      delete legacy.plain_summary;
    } else {
