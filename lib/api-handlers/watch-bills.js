...
async function fetchSummaryExcerpt(bill, apiKey) {
  // Prefer Claude-processed vote card when we already synced this bill.
  try {
    const type = String(bill.type || "").toLowerCase();
    const number = String(bill.number || "");
    if (type && number && bill.congress) {
      const rows = await supabaseRest(
        `processed_votes?congress=eq.${Number(bill.congress)}&bill_type=eq.${encodeURIComponent(
          type
        )}&legislation_number=eq.${encodeURIComponent(
          number
        )}&summary=not.is.null&select=summary,vote_kind,vote_date&order=vote_date.desc&limit=12`
      );
      if (Array.isArray(rows) && rows.length) {
        const finalPassage = rows.find(
          (row) =>
            String(row.vote_kind || "").toLowerCase() === "final_passage" &&
            String(row.summary || "").trim()
        );
        const best =
          finalPassage ||
          rows.find((row) => String(row.summary || "").trim()) ||
          null;
        const summary = String(best?.summary || "").trim();
        if (summary) return truncateSentences(summary, 3);
      }
    }
  } catch (error) {
    console.warn("processed_votes excerpt lookup failed:", error);
  }
