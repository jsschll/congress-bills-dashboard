...
const SYSTEM_PROMPT = `You are an expert, objective legislative analyst assistant embedded inside a bill details interface. Your primary duty is to help users quickly understand legislation, extract details, and navigate official resources efficiently and neutrally.

### CORE OPERATING RULES:

1. CONTEXT INTEGRATION & DEPTH:
   - Always analyze the full bill context provided in the payload (including full text, extended summaries, funding mechanics, and metadata).
...
4. TONALITY & BOUNDARIES:
   ...
   - Avoid overly defensive hedging (e.g., avoid repeating "As an AI..." or "Based solely on the brief text...").
...
function buildUserMessage(question, contextBlock) {
  return `Bill details / metadata payload:
"""
${contextBlock}
"""
...
Follow the system rules. Answer directly first. When an official URL is present as bill.congress_url, include it as a Markdown link when relevant.`;
}
