...
async function lookupRepresentativesByLocation(options = {}) {
  ...
  // Keep scorecard profiles fresh from Geocodio legislators on each lookup.
  if (Array.isArray(location.legislators) && location.legislators.length) {
    try {
      await upsertRepresentativeProfiles(supabase, location.legislators);
    } catch (upsertError) {
      console.warn(
        "Geocodio profile upsert skipped:",
  ...
  try {
    ...
    // If scorecard table is empty for this state, backfill from politicians roster.
    if (!profiles.length) {
      await syncProfilesFromPoliticians(supabase, { state: location.state });
      profiles = await fetchFederalRepresentatives(supabase, location);
    }
  } catch (dbError) {
