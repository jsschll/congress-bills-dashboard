...
const { fetchLiveScorecardVotes, enrichScorecardVotesWithClaude } = require("../scorecard-live-votes");
  ...
  return [...list]
    .map((item) =>
      enrichDonorSource({
        ...
        industry_slug: item?.industry_slug || item?.industrySlug,
 ...
 */
async function fetchCampaignFinance(supabase, profile) {
  ...
  let row = data || null;
  if (financeNeedsRefresh(row)) {
    const synced = await syncCampaignFinanceFromFec(supabase, profile);
...
async function fetchRecentVotes(supabase, politicianId, limit = 5) {
  // Over-fetch so we can drop legacy Seed/Placeholder rows and still fill the list.
...
/**
 * Detect legacy Seed / Placeholder Truth-in-Voting rows.
 * @param {object|null|undefined} billRow
 */
function isPlaceholderBill(billRow) {
  if (!billRow) return true;
  const title = String(billRow.title || "");
  const number = String(billRow.bill_number || billRow.billNumber || "");
  const summary = String(
    billRow.plain_english_summary || billRow.plainEnglishSummary || ""
  );
  if (/^seed\s*:/i.test(title) || /^placeholder\s*:/i.test(title)) return true;
  if (/-seed-/i.test(number) || /-ph-/i.test(number)) return true;
  if (/seeded placeholder/i.test(summary)) return true;
  if (/placeholder vote data/i.test(summary)) return true;
  if (/placeholder (house|senate|vote|roll)/i.test(summary)) return true;
  return false;
}

function isPlaceholderVoteRow(row) {
  const billRow = Array.isArray(row?.bill) ? row.bill[0] : row?.bill;
  return isPlaceholderBill(billRow);
}
...
function buildRecentVotePayload(row) {
  ...
  return {
    ...
    industryTags: bill?.industryTags || [],
...
async function buildScorecardPayload(supabase, profileRow, options = {}) {
  ...
  const [financeRow, attendanceRow, voteRows] = await Promise.all([
    fetchCampaignFinance(supabase, profileRow),
    fetchAttendance(supabase, profile.id),
    fetchRecentVotes(supabase, profile.id, voteLimit),
  ]);

  const realVoteRows = (voteRows || [])
    .filter((row) => !isPlaceholderVoteRow(row))
    .slice(0, voteLimit);
  let recentVotes = realVoteRows.map(buildRecentVotePayload);

  const allowLive = options.liveVotes !== false;
  if (!recentVotes.length && allowLive && profile.bioguideId) {
    try {
      recentVotes = await fetchLiveScorecardVotes({
        bioguideId: profile.bioguideId,
        chamber: profile.chamber,
        limit: Math.min(voteLimit, 8),
      });
    } catch (error) {
      console.warn(
        "live scorecard votes failed:",
        error?.message || error
      );
      recentVotes = [];
    }
  } else if (recentVotes.length) {
    // DB-backed votes still get Claude overlay when available.
    try {
      recentVotes = await enrichScorecardVotesWithClaude(recentVotes, {
        generate: true,
      });
    } catch (error) {
      console.warn(
        "scorecard Claude enrich failed:",
        error?.message || error
      );
    }
  }
...
/**
 * Map a politicians-roster row into an upsert payload for representative_profiles.
 * @param {object} row
 */
function mapPoliticianRosterToLegislator(row) {
  if (!row) return null;
  const chamberRaw = String(row.chamber || "").toLowerCase();
  const chamber =
    chamberRaw === "senate"
      ...
      : chamberRaw === "house"
        ? "House"
        : null;
  if (!chamber || !row.name) return null;
  const bioguide = row.bioguide_id
    ? String(row.bioguide_id).toUpperCase()
    : null;
  return {
    ...
    party: row.party || null,
    district:
      chamber === "Senate" ? null : normalizeDistrict(row.district) || null,
    state: row.state ? String(row.state).toUpperCase() : null,
    chamber,
    phone: row.phone || null,
    website: row.website_url || null,
    photo_url:
      ...
      (bioguide
        ? `https://www.congress.gov/img/member/${bioguide.toLowerCase()}_200.jpg`
        : null),
    politician_id: row.id || null,
  };
}

/**
 * Seed / resolve a scorecard profile from the politicians roster.
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {{ bioguideId?: string|null, politicianId?: string|null }} options
 */
async function seedScorecardFromRoster(supabase, options = {}) {
  const bioguideId = String(options.bioguideId || "").trim() || null;
  const politicianId = String(options.politicianId || "").trim() || null;
  if (!bioguideId && !politicianId) return null;
  ...
  if (politicianId) query = query.eq("id", politicianId);
  ...
  if (!isFederalLegislator && !row.bioguide_id) return null;

  const mapped = mapPoliticianRosterToLegislator(row);
  if (!mapped) return null;
  const seeded = await upsertRepresentativeProfiles(supabase, [mapped]);
...
/**
 * Map a Congress.gov /v3/member payload into a representative_profiles upsert row.
 * @param {object} member
 */
function mapCongressMemberToLegislator(member) {
  ...
  const chamberRaw = String(
    latest.chamber || member.chamber || ""
  ).toLowerCase();
  ...
  const district =
    chamber === "Senate"
      ? null
      : normalizeDistrict(member.district ?? latest.district) || null;
...
/**
 * Seed a scorecard profile from Congress.gov when the local roster lacks the member.
 * Covers Action Match / who-voted deep links for any bioguide.
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {string} bioguideId
 */
async function seedScorecardFromCongressMember(supabase, bioguideId) {
  const bio = String(bioguideId || "")
    .trim()
    .toUpperCase();
  if (!bio) return null;
  ...
  if (!apiKey) {
    console.warn(
      "Scorecard Congress.gov seed skipped: missing CONGRESS_API_KEY"
    );
    return null;
  }

  const url = `${CONGRESS_API}/member/${encodeURIComponent(
    bio
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `Congress.gov member ${bio} failed (${response.status}): ${text.slice(0, 160)}`
    );
  }
  const payload = await response.json();
  const mapped = mapCongressMemberToLegislator(payload?.member || payload);
  if (!mapped) return null;
  const seeded = await upsertRepresentativeProfiles(supabase, [mapped]);
  return seeded[0] || null;
}
...
/**
 * Load one scorecard by representative profile id, bioguide, or politicians roster id.
 * Auto-seeds from the politicians roster, then Congress.gov, when missing.
 * @param {{ id?: string|null, bioguideId?: string|null, politicianId?: string|null, supabase?: any, voteLimit?: number }} options
 */
async function getScorecardById(options = {}) {
  ...
  if (!id && !bioguideId && !politicianId) {
    const error = new Error("Provide id, bioguideId, or politicianId");
  ...
  // Politicians-tab / deep links: seed federal roster rows into scorecard tables.
  if (!data) {
    try {
      data = await seedScorecardFromRoster(supabase, {
        bioguideId,
        politicianId: politicianId || id,
      });
    } catch (seedError) {
      console.warn(
        "Scorecard roster seed skipped:",
        seedError.message || seedError
      );
    }
  }

  // Action Match / who-voted lists link any roll-call member by bioguide —
  // most are not in the thin local roster, so fall back to Congress.gov.
  if (!data && bioguideId) {
    try {
      data = await seedScorecardFromCongressMember(supabase, bioguideId);
    } catch (seedError) {
      console.warn(
        "Scorecard Congress.gov seed skipped:",
        seedError.message || seedError
      );
    }
  }
  ...
  if (Array.isArray(location.legislators) && location.legislators.length) {
    try {
      ...
      if (missing.length) {
        ...
        // Profiles exist — still ensure donor/attendance placeholders.
  ...
  for (const profile of profiles) {
    try {
      representatives.push(
        await buildScorecardPayload(supabase, profile, {
          voteLimit: 25,
          // Live Congress.gov lookups are too slow for multi-rep ZIP responses;
          // the scorecard UI hydrates Truth in Voting from /api/politician-profile.
          liveVotes: false,
        })
      );
    } catch (scoreError) {
...
module.exports = {
  ...
  mapPoliticianRosterToLegislator,
  normalizeDistrict,
  orderRepresentativesForTabs,
  seedScorecardFromRoster,
  seedScorecardFromCongressMember,
  mapCongressMemberToLegislator,
  sortIndustries,
  syncProfilesFromPoliticians,
  upsertRepresentativeProfiles,
  ensureScorecardPlaceholders,
  fetchLiveScorecardVotes: require("../scorecard-live-votes").fetchLiveScorecardVotes,
  isPlaceholderBill,
  isPlaceholderVoteRow,
};
