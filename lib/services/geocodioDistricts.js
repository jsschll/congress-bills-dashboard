...
function mockResolveDistrict(query) {
  ...
  if (zipCode && MOCK_ZIP_DISTRICTS[zipCode]) {
    ...
    return {
      ...
      districts: [normalizeDistrict(hit.district)].filter(Boolean),
...
function mapGeocodioLegislator(legislator, state, districtNumber) {
  ...
  const chamber =
    type === "senator" ? "Senate" : type === "representative" ? "House" : null;
  const bioguide = references.bioguide_id
    ? String(references.bioguide_id).toUpperCase()
    : null;
  const name = fullName(bio) || "Unknown member";
  const district =
    chamber === "Senate"
      ? null
      : normalizeDistrict(districtNumber) || null;

  return {
    bioguide_id: bioguide,
    name,
    party: bio.party || null,
    district,
    state: state || null,
    chamber,
    office_address: contact.address || null,
...
async function resolveCongressionalDistrict(input = {}) {
  ...
  if (!apiKey) {
    return { ...mockResolveDistrict(query), legislators: [], districts: [] };
  }
  ...
  for (const cd of districts) {
    const districtNumber = normalizeDistrict(
      cd.district_number ?? cd.name ?? cd.district
    );
    if (districtNumber) districtNumbers.push(districtNumber);
    for (const legislator of cd.current_legislators || []) {
      const mapped = mapGeocodioLegislator(legislator, state, districtNumber);
      const key =
        mapped.bioguide_id ||
        `${mapped.chamber}:${mapped.name}:${mapped.district || "statewide"}`;
      if (seen.has(key)) continue;
      seen.add(key);
      if (mapped.bioguide_id) bioguides.push(mapped.bioguide_id);
      if (mapped.chamber) legislators.push(mapped);
    }
  }
  ...
  return {
    state,
    district: district || districtNumbers[0] || "AL",
    districts: [...new Set(districtNumbers)],
