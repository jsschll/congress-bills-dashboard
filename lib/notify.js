...
async function sendEmail({ to, subject, html, text }) {
  ...
  const from = env("NOTIFY_FROM_EMAIL", "EMAIL_FROM") || "Article 1 <onboarding@resend.dev>";
  if (!apiKey) {
...
function notificationEmailHtml(item, { heading } = {}) {
  ...
  const feedUrl = `${siteBaseUrl()}/bills-policies.html?tab=mine`;
  return `
    <div style="font-family: Georgia, serif; line-height: 1.5; color: #111;">
      <h1 style="font-size: 20px;">${heading || "Article 1 update"}</h1>
...
function digestEmailHtml(items, periodLabel) {
  ...
  return `
    <div style="font-family: Georgia, serif; line-height: 1.5; color: #111;">
      <h1 style="font-size: 20px;">Your ${escapeHtml(periodLabel)} Article 1 digest</h1>
