...
function mapProcessedVoteToFeedItem(row = {}) {
  ...
  const primaryCategory = String(row.primary_category || "").trim();
  const keyPoints = Array.isArray(row.key_points)
    ? row.key_points.filter(Boolean).map((entry) => String(entry).trim())
    : Array.isArray(row.key_impacts)
      ? row.key_impacts.filter(Boolean).map((entry) => String(entry).trim())
      : [];
  const billType = normalizeLegislationType(row.bill_type);
  const legislationNumber = String(
    row.legislation_number || parseBillNumberParts(billNumber)?.legislationNumber || ""
  ).replace(/\D/g, "");
  const federalBillId =
    congress && billType && legislationNumber
      ? `federal-${congress}-${billType}-${legislationNumber}`
      : "";

  return {
    id: federalBillId || rollCallId || billId,
    billId: federalBillId || billId || rollCallId,
    rollCallId,
    ...
    keyPoints,
    key_impacts: keyPoints,
    keyImpacts: keyPoints,
