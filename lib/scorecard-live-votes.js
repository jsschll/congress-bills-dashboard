...
function formatBillTitle(billNumber, title) {
  ...
  if (rawTitle.toLowerCase().startsWith(compact.toLowerCase())) {
    return rawTitle.includes(":") ? rawTitle : `${compact}: ${rawTitle.slice(compact.length).trim() || rawTitle}`;
  }
  return `${compact}: ${rawTitle}`;
}

function mapLiveCardToScorecard(card) {
  const billNumber = card.billNumber || null;
  const title = formatBillTitle(
    billNumber,
    card.title || card.voteQuestion || "Congressional roll call"
  );
  const summary =
    String(card.shortPitch || card.officialSummary || card.voteQuestion || "")
      .trim() || null;
  return {
    votePosition: castToVotePosition(card.voteCast),
    ...
    category:
      card.subjectCategory && card.subjectCategory !== "Other"
        ...
        : inferCategory(card.title, card.voteQuestion),
    voteDate: displayDate(card.date || card.lastUpdated) || null,
    impacts: {
      wallet: null,
      community: null,
      rights: null,
...
async function fetchHouseScorecardVotes(apiKey, bioguideId, limit = 5) {
  ...
  return found.map(mapLiveCardToScorecard);
}
...
async function fetchSenateScorecardVotes(bioguideId, limit = 5) {
  ...
  try {
    ...
    return (cards || []).map(mapLiveCardToScorecard);
  } catch (error) {
    console.warn(
