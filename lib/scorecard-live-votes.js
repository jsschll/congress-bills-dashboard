...
const { describeVoteMotion } = require("./vote-motion-label");
...
/**
 * Congress.gov house-vote LIST omits voteQuestion; DETAIL includes it
 * (e.g. "On Passage", "On Motion to Recommit").
 */
async function fetchHouseVoteQuestion(apiKey, congress, session, roll) {
  if (!apiKey || !congress || !roll) {
    return { voteQuestion: "", voteType: "", result: "", legislationTitle: "" };
  }
  try {
...
function mapLiveCardToScorecard(card) {
  ...
  const voteQuestion = String(
    card.voteQuestion || card.vote_question || ""
  ).trim();
  const voteKind = String(card.voteKind || card.vote_kind || "").trim() || null;
  const motion = describeVoteMotion({
    voteQuestion,
    voteKind,
    result: card.result || "",
  });
  return {
    ...
    rollCallNumber:
      card.rollCallNumber != null
        ? Number(card.rollCallNumber)
        : card.roll_call_number != null
          ? Number(card.roll_call_number)
          : null,
    billNumber,
    title,
    rawTitle,
    ...
    is_key_vote:
      typeof card.is_key_vote === "boolean"
        ? card.is_key_vote
        : typeof card.isKeyVote === "boolean"
          ? card.isKeyVote
          : null,
    isKeyVote:
      typeof card.is_key_vote === "boolean"
    ...
    motionDetail: motion.detail,
    isProceduralMotion: motion.isProcedural,
...
async function enrichScorecardVotesWithClaude(votes = [], options = {}) {
  ...
  for (const vote of list) {
    ...
    // Refresh motion badge after any vote_question backfill.
    const motion = describeVoteMotion(vote);
    vote.voteQuestion = motion.voteQuestion || vote.voteQuestion || null;
    vote.motionLabel = motion.label;
    vote.motionDetail = motion.detail;
    vote.isProceduralMotion = motion.isProcedural;
  }
    ...
    await Promise.all(
      chunk.map(async (vote) => {
        try {
          ...
          vote.is_key_vote = card.is_key_vote !== false;
          vote.isKeyVote = vote.is_key_vote;
    ...
    const results = await Promise.all(
      chunk.map(async ({ raw: vote, voteKind: listedKind }) => {
        ...
        try {
          ...
          // Keep bill-linked motions (recommit, etc.) so YES/NO conflicts are
          // explainable; drop only pure chamber noise once we know the question.
          if (
          ...
          return {
            ...
            subjectCategory: inferCategory(title, voteQuestion),
