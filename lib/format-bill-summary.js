...
- MAXIMUM LENGTH: 2 short sentences (~35 words max) for summary / plain_summary.
...
Keep summary and plain_summary to ≤2 short sentences (~35 words). Prefer dollars, agencies, rules, and penalties from the text over vague phrases like "provides funding for" or "addresses". Stay specific to the text.
...
/** Soft word cap for punchy Action Match summaries (≤2 sentences / ~35 words). */
function clampPlainSummaryWords(text = "", maxWords = 35) {
...
function clampImpactWords(text = "", maxWords = 14) {
  ...
  const plain_summary =
    clampPlainSummaryWords(
      completeSentences(raw.plain_summary || raw.what_it_does || "", {
        ...
        completeSentences(summary, { maxSentences: 2, maxChars: 220 }) ||
        summary,
      35
    ) || summary;
  const yea_impact =
