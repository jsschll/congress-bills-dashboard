...
const DEFAULT_YEA_LABEL = "Yes, support it";
const DEFAULT_NAY_LABEL = "No, oppose it";
const DEFAULT_YEA_MEANS =
  "Voting Yes means you want this to move forward as written.";
const DEFAULT_NAY_MEANS = "Voting No means you want to stop this.";
...
const SYSTEM_PROMPT = `You write vote cards for everyday people who do not follow politics.
...
2. Write at about an 8th-grade reading level. Prefer short words. Explain what this would change in daily life when the text allows.
3. Avoid insider jargon: do not say “measure”, “roll call”, “cloture”, “motion to proceed”, “concurring”, “as amended”, “provided that”, “notwithstanding”, or section cites. Prefer “bill”, “change to the bill”, “Yes”, and “No”.
4. If the text is thin, unclear, or mostly procedural, say what you can neutrally and set yea_label to "Yes, support it" and nay_label to "No, oppose it".
5. summary must be 1–2 COMPLETE sentences (never cut off mid-sentence, never paste truncated legalese).
6. yea_means / nay_means must start with “Voting Yes…” / “Voting No…” and describe the real-world outcome on THIS bill — not a stock “You support ending…” template.
7. Labels are 2–4 words, parallel, concrete. Only use vivid verbs like End/Keep/Pass/Reject when the text clearly supports them; otherwise use Yes, support it / No, oppose it.
8. No slogans and no fear-mongering.
...
function buildUserPrompt(rawSummary, billTitle) {
  const title = String(billTitle || "").trim() || "Untitled bill";
  const summary = String(rawSummary || "").trim() || "(No official summary available.)";
...
{
  "summary": "This bill would end federal money that helps people buy energy-saving home appliances and train local contractors.",
  "yea_means": "Voting Yes means you want those rebate and training programs to end.",
  "nay_means": "Voting No means you want those rebate and training programs to stay available.",
...
If unsure, set confident=false and use Yes, support it / No, oppose it labels.
...
function normalizeCard(raw = {}, fallbackTitle = "", sourceText = "") {
  ...
  if (!summary) {
    summary = fallbackTitle
      ? `This Yes/No vote is about ${simplifyBillTitle(fallbackTitle)}.`
...
function simplifyBillTitle(title = "") {
  ...
  for (let i = 0; i < 2; i += 1) {
    text = text
      .replace(/^(an?\s+)?(original\s+)?bill\s+(to\s+)?/i, "")
      .replace(/^(an?\s+)?act\s+(to\s+)?/i, "")
      .replace(/^(a\s+)?joint resolution\s+(to\s+)?/i, "")
      .replace(/^(a\s+)?resolution\s+(to\s+)?/i, "")
      .trim();
  }
  text = text
...
/**
 * Everyday-language fallback when CRS/LLM text is missing.
 */
function plainVoteFallback(vote = {}) {
  const { actors, chamber } = chamberActors(vote);
  ...
  if (
    ...
    about = /^to\s+/i.test(cleanTopic)
      ? ` It would ${cleanTopic.replace(/^to\s+/i, "")}.`
      : ` It covers ${cleanTopic}.`;
  }
  ...
  if (vote.voteKind === "final_passage" || /\bon passage\b/.test(question)) {
    return `${actors} took a final Yes/No vote on whether to pass ${bill}.${about}`;
  }
  if (
    ...
    return `${actors} voted on whether to accept the other chamber’s version of ${bill}.${about}`;
  }
  if (vote.voteKind === "amendment" || /\bamendment\b|\bamdt\b/.test(question)) {
    return `${actors} voted on a proposed change to ${bill}.${about}`;
  }
  if (/joint resolution/.test(question)) {
    return `${actors} voted on ${bill}.${about}`;
  }
  if (vote.voteQuestion) {
    const cleaned = collapseWhitespace(vote.voteQuestion)
      .replace(/^on the\s+/i, "")
      .replace(/^on\s+/i, "");
    return `${chamber} Yes/No vote on ${cleaned}.${about}`;
  }
  return `${actors} took a recorded Yes/No vote on ${bill}.${about}`;
}
...
function heuristicFormat(rawSummary, billTitle = "", voteMeta = null) {
  ...
  const body = collapseWhitespace(rawSummary)
    .replace(/\b(provided that|notwithstanding|herein|thereof|thereunder)\b/gi, "")
    .replace(/\s+/g, " ")
    .trim();
  let summary = completeSentences(body, { maxSentences: 2, maxChars: 420 });
  ...
  if (
    ...
    summary = voteMeta
      ...
      : title
        ? `This Yes/No vote is about ${simplifyBillTitle(title)}.`
        : DEFAULT_VOTE_SUMMARY;
  }
