...
function buildCacheKey(query) {
  ...
  // Version prefix invalidates older cached rosters that missed city mayors.
  const version = "v2";
  if (/^\d{5}(-\d{4})?$/.test(digitsOnly)) {
    const zip = digitsOnly.slice(0, 5);
    return { cacheKey: `${version}:zip:${zip}`, zipCode: zip, queryRaw: raw };
  }
  const zipCode = extractZip(collapsed);
  return {
    cacheKey: `${version}:addr:${collapsed}`,
...
async function fetchLocalOfficialsForGeography(supabase, geography = {}) {
  ...
  // Fuzzy fallback: "Saint Paul" vs "St. Paul", partial city labels, etc.
  const { data: fuzzy, error: fuzzyError } = await supabase
    ...
    .limit(50);
  if (fuzzyError) {
    console.warn("local_officials fuzzy error:", fuzzyError.message || fuzzyError);
    return [];
  }
  return (fuzzy || []).map(mapLocalOfficialRow).filter((row) => row?.name);
}

function officeTitleOf(person) {
  return String(
    person?.office_title ||
...
function isMayorPerson(person) {
  ...
  const title = officeTitleOf(person);
  ...
  // Cicero LOCAL_EXEC is the municipal executive — usually the mayor.
  if (districtType.includes("LOCAL_EXEC")) return true;
  return false;
}

/** Ensure live city executives land in the City → Mayor section. */
function promoteCityMayors(politicians = []) {
  return (politicians || []).map((person) => {
    if (!isMayorPerson(person)) return person;
    const title =
      String(person.office_title || person.metadata?.office_title || "").trim() ||
      "Mayor";
    return {
      ...person,
      level: "city",
...
/**
 * When Cicero (or another live source) finds a mayor for a city that is not in
 * our seed table yet, save them so future lookups show Mayor without waiting
 * on another Civic API round-trip.
 */
async function persistDiscoveredMayors(supabase, politicians, geography = {}) {
  if (!supabase) return;
  const mayors = (politicians || []).filter(isMayorPerson);
  for (const mayor of mayors) {
    const name = String(mayor.name || "").trim();
    if (!name || name === "Official" || name === "Unknown") continue;

    const stateCode = normalizeStateCode(geography.state || mayor.state);
    ...
    // Skip district-looking values that are not city names.
    if (/^\d+$/.test(cityName)) continue;

    const title =
      /mayor/i.test(String(mayor.office_title || mayor.metadata?.office_title || ""))
        ? String(mayor.office_title || mayor.metadata?.office_title).trim()
        : "Mayor";
    ...
    try {
      const { data: existing, error: findError } = await supabase
        ...
        .limit(1);
      if (findError) {
        console.warn("persist mayor lookup failed:", findError.message || findError);
        continue;
      }
      ...
      const payload = {
        ...
        city_name: cityName,
        party: mayor.party || null,
        photo_url: mayor.photo_url || null,
        website_url: mayor.website_url || null,
        source_name: mayor.source || "lookup",
        source_ref: mayor.external_key || null,
      ...
      if (existing?.[0]?.id) {
        const { error } = await supabase
          ...
          .eq("id", existing[0].id);
        if (error) {
          console.warn("persist mayor update failed:", error.message || error);
        }
      } else {
        const { error } = await supabase.from("local_officials").insert(payload);
        if (error) {
          console.warn("persist mayor insert failed:", error.message || error);
        }
      }
    } catch (error) {
      console.warn("persist mayor failed:", error.message || error);
    }
  }
}
...
async function enrichLookupRoster(supabase, basePayload) {
  ...
  const politicians = promoteCityMayors(basePayload.politicians || []);

  // Grow the mayor directory from live Civic results for cities not in the seed.
  try {
    await persistDiscoveredMayors(supabase, politicians, geography);
  } catch (error) {
    console.warn("mayor persist skipped:", error.message || error);
  }
