...
5. SQL Editor: run [`supabase/schema.sql`](supabase/schema.sql)
   ...
   - For Representative Scorecard tables + types, run [`supabase/migration-representative-scorecard.sql`](supabase/migration-representative-scorecard.sql) (see `types/scorecard.ts`). Enables `GET /api/representatives/lookup?zipCode=` / `?address=`. Lookups auto-seed missing profiles + placeholder donor/attendance/votes from Geocodio. Backfill Texas (or `--state=`) with `npm run seed:federal-reps`.
