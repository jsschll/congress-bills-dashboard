...
--      -> appellate_district_number, judicial_district_numbers[]
-- 3) state_officials where:
--      level = 'Statewide'
--      OR (level = 'Appellate' AND district_number IN appellate)
--      OR (level = 'District' AND district_number IN judicial array)
--      OR (level = 'County' AND county_name matches)
--
...
create table if not exists public.county_district_mapping (
  ...
  appellate_district_number text,
...
create table if not exists public.state_officials (
  ...
  level text not null check (
    level in ('Statewide', 'Appellate', 'District', 'County')
  ),
  state_code text not null,
